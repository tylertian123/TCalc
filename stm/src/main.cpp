...
void normalKeyPressHandler(uint16_t key) {
    if (exprEditMode == ExprEditMode::RESULT) {
        ...
        default: {
            ...
            if (key == KEY_PLUS || key == KEY_MINUS || key == KEY_MUL || key == KEY_DIV || key == KEY_FRAC ||
                    key == KEY_EQUAL) {
            ...
            if (isFunc) {
                ...
                if (end == equalsIndex) {
                    ...
                    if (argc > 0xFF) {
                        ...
                        // Make a new array with the argument names since the util::DynamicArray's contents will be
                        // automatically freed
                ...
                // Evaluate
                util::DynamicArray<neda::NEDAObj *> val = util::DynamicArray<neda::NEDAObj *>::createConstRef(
                        expr->contents.begin() + equalsIndex + 1, expr->contents.end());
...
int main() {
    ...
    display.drawString(lcd::SIZE_WIDTH / 2, 25, "TCalc " VERSION_STR,
            lcd::DrawBuf::FLAG_INVERTED | lcd::DrawBuf::FLAG_HALIGN_CENTER);
#ifdef _TEST_MODE
