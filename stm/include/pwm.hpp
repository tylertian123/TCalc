...
namespace pwm {
    ...
    void (*const PWMOutput<CHANNEL, CLOCK_FREQ, PWM_FREQ, PWM_STEPS>::TIM_OCInitFuncs[4])(
            TIM_TypeDef *, TIM_OCInitTypeDef *) = {TIM_OC1Init, TIM_OC2Init, TIM_OC3Init, TIM_OC4Init};
    template <uint8_t CHANNEL, uint32_t CLOCK_FREQ, uint32_t PWM_FREQ, uint16_t PWM_STEPS>
    void (*const PWMOutput<CHANNEL, CLOCK_FREQ, PWM_FREQ, PWM_STEPS>::TIM_SetCompareFuncs[4])(
            TIM_TypeDef *, uint16_t) = {TIM_SetCompare1, TIM_SetCompare2, TIM_SetCompare3, TIM_SetCompare4};
} // namespace pwm
